...
const EXCEL_SHEET = "l4e3_questions";
...
const OPTIONAL_MATCH_TOKENS = new Set(["a", "an", "the", "our", "their", "his", "her", "its", "my", "your", "this", "that", "these", "those", "all"]);
...
let finalDragTokenIdx = null;

let subcategory = "Grammar";
let level = "Basic";
let day = "313";
let quizTitle = "quiz_Grammar_Basic_313";
let userId = "";
let lessonTitle = "";
let exerciseTitle = "";
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!questions.length) {
    renderEmpty("No L4-E3 rows found.");
    return;
  }
...
function renderIntro() {
  ...
  mount.innerHTML = `
    <div class="box">
      <div style="font-size:18px; font-weight:900; color:#7e3106; margin-bottom:10px;">Pleks L4-E3 (Draft)</div>
      ...
      <div style="font-size:12px; color:#7e3106; line-height:1.35; letter-spacing:-0.2px;">
        Drag Korean fragments to the matching part of the full sentence.
      </div>
...
function parseGuidedQuestionField(raw) {
  ...
  parts.forEach((part) => {
    ...
    const enRaw = sanitizeEnFragmentText(m ? m[1] : body).replace(/Korean sentence:.*$/i, "").replace(/Sentence:.*$/i, "");
    const koRaw = compactWhitespace(m ? m[2] : "")
      .replace(/Korean sentence:.*$/i, "")
      .replace(/English sentence:.*$/i, "")
      .replace(/Sentence:.*$/i, "");
    const en = compactWhitespace(enRaw);
    const ko = compactWhitespace(koRaw);
    if (!en) return;
...
function findSequenceMatches(answerTokens, fragmentTokens) {
  const ans = (answerTokens || []).map((t) => normalizeWord(t)).filter(Boolean);
  const frag = (fragmentTokens || []).map((t) => normalizeWord(t)).filter(Boolean);
  if (!ans.length || !frag.length || frag.length > ans.length) return [];
  const out = [];
  for (let i = 0; i <= ans.length - frag.length; i += 1) {
    let ok = true;
    for (let j = 0; j < frag.length; j += 1) {
      if (ans[i + j] !== frag[j]) {
        ok = false;
        break;
      }
    ...
    if (!ok) continue;
    out.push(Array.from({ length: frag.length }, (_, k) => i + k));
  }
  return out;
}

function findOrderedSubsequenceMatches(answerTokens, fragmentTokens) {
  const ans = (answerTokens || []).map((t) => normalizeWord(t)).filter(Boolean);
  const frag = (fragmentTokens || []).map((t) => normalizeWord(t)).filter(Boolean);
  if (!ans.length || !frag.length || frag.length > ans.length) return [];
  const out = [];

  for (let start = 0; start < ans.length; start += 1) {
    if (ans[start] !== frag[0]) continue;
    const indices = [start];
    let scan = start + 1;
    let ok = true;
    for (let k = 1; k < frag.length; k += 1) {
      const token = frag[k];
      let found = -1;
      for (let j = scan; j < ans.length; j += 1) {
        if (ans[j] === token) {
          found = j;
          break;
        }
    ...
    if (ok) out.push(indices);
  }
  return out;
}

function chooseBestMatch(matches, preferredStart) {
  if (!Array.isArray(matches) || !matches.length) return [];
  if (!Number.isInteger(preferredStart)) return matches[0];
  const forward = matches.filter((m) => Number.isInteger(m?.[0]) && m[0] >= preferredStart);
  const base = forward.length ? forward : matches;
  return base.slice().sort((a, b) => {
    const spanA = (Number(a?.[a.length - 1]) || 0) - (Number(a?.[0]) || 0);
    const spanB = (Number(b?.[b.length - 1]) || 0) - (Number(b?.[0]) || 0);
    if (spanA !== spanB) return spanA - spanB;
    return (Number(a?.[0]) || 0) - (Number(b?.[0]) || 0);
  })[0];
}

function fillMissingFragmentIndices(answerTokens, fragments) {
  if (!Array.isArray(fragments) || !fragments.length) return;
  let cursor = 0;
  const ordered = fragments.slice().sort((a, b) => Number(a?.no) - Number(b?.no));
  ordered.forEach((frag) => {
    if (Array.isArray(frag?.sentenceIndices) && frag.sentenceIndices.length) {
      cursor = Number.isInteger(frag.sentenceIndices[0]) ? frag.sentenceIndices[0] : cursor;
      return;
    }
    const fragTokens = splitSpaceTokens(String(frag?.en || ""));
    let matches = findSequenceMatches(answerTokens, fragTokens);
    if (!matches.length) {
      matches = findOrderedSubsequenceMatches(answerTokens, fragTokens);
    }
    if (!matches.length) {
      const relaxed = fragTokens.filter((t) => !OPTIONAL_MATCH_TOKENS.has(normalizeWord(t)));
      if (relaxed.length && relaxed.length < fragTokens.length) {
        matches = findOrderedSubsequenceMatches(answerTokens, relaxed);
      }
    }
    const picked = chooseBestMatch(matches, cursor);
    frag.sentenceIndices = Array.isArray(picked) ? picked : [];
    if (frag.sentenceIndices.length) cursor = frag.sentenceIndices[0];
  });
}
...
function buildQuestions(rows) {
  ...
  return selected
    .map((row, i) => {
      ...
      fillMissingFragmentIndices(answerTokens, fragments);
...
function createQuestionState() {
  return {
    ...
    finalArrange: null, // { chipOrder:number[], pickedIdxByFragId:Record<number, number[]> }
  };
}
...
function initFinalArrangeState(q, state) {
  ...
  const chipOrder = (q.fragments || [])
    .slice()
    .sort((a, b) => Number(a?.no) - Number(b?.no))
    .map((f) => Number(f.id))
    .filter((n) => Number.isInteger(n));
  const pickedIdxByFragId = {};
  chipOrder.forEach((fid) => {
    pickedIdxByFragId[fid] = [];
  });
  state.finalArrange = {
    chipOrder,
    pickedIdxByFragId,
...
function getFinalTokenOwners(q, finalState, tokenIdx) {
  ...
  const picked = finalState.pickedIdxByFragId || {};
  const owners = [];
  (q.fragments || []).forEach((frag) => {
    const fid = Number(frag.id);
    const list = Array.isArray(picked[fid]) ? picked[fid] : [];
    if (list.includes(Number(tokenIdx))) owners.push(fid);
  });
  return owners;
}

function getRequiredFragIndices(frag) {
  return Array.from(new Set((Array.isArray(frag?.sentenceIndices) ? frag.sentenceIndices : []).map((n) => Number(n)).filter((n) => Number.isInteger(n)))).sort((a, b) => a - b);
}

function getPickedFragIndices(finalState, fragId) {
  const picked = finalState?.pickedIdxByFragId || {};
  return Array.from(new Set((Array.isArray(picked[fragId]) ? picked[fragId] : []).map((n) => Number(n)).filter((n) => Number.isInteger(n)))).sort((a, b) => a - b);
}

function isSameNumberSet(a, b) {
  const aa = Array.from(new Set((a || []).map((n) => Number(n)).filter((n) => Number.isInteger(n)))).sort((x, y) => x - y);
  const bb = Array.from(new Set((b || []).map((n) => Number(n)).filter((n) => Number.isInteger(n)))).sort((x, y) => x - y);
...
function isFinalFragSolved(finalState, frag) {
  const fid = Number(frag?.id);
  if (!Number.isInteger(fid)) return false;
  const required = getRequiredFragIndices(frag);
  const picked = getPickedFragIndices(finalState, fid);
  if (!required.length) return picked.length > 0;
  return isSameNumberSet(required, picked);
}

function countSolvedFinalFrags(q, finalState) {
  return (q?.fragments || []).filter((frag) => isFinalFragSolved(finalState, frag)).length;
}

function getRequiredOwnersForToken(q, tokenIdx) {
  const idx = Number(tokenIdx);
  if (!q || !Number.isInteger(idx)) return [];
  return (q.fragments || [])
    .filter((frag) => getRequiredFragIndices(frag).includes(idx))
    .map((frag) => Number(frag.id))
    .filter((n) => Number.isInteger(n));
}

function isFinalTokenFullySolved(q, finalState, tokenIdx) {
  const requiredOwners = getRequiredOwnersForToken(q, tokenIdx);
  if (!requiredOwners.length) return false;
  const actualOwners = getFinalTokenOwners(q, finalState, tokenIdx);
  return requiredOwners.every((fid) => actualOwners.includes(fid));
}

function buildFinalSentenceTokenStyle(owners) {
  const ids = Array.isArray(owners) ? owners.slice() : [];
  if (!ids.length) return "";
  const main = pickFragColor(Number(ids[ids.length - 1] ?? 0)).solid;
  const border = mixHex(main, "#ffffff", 0.2);
  const ring = rgbaFromHex(main, ids.length >= 2 ? 0.42 : 0.28);
  return `--pill-bg:#ffffff; --pill-bd:${border}; --pill-fg:#544433; box-shadow:0 0 0 2px ${ring};`;
}
  ...
  if (stageMode === "merge") {
    ...
    mount.innerHTML = `
      <div class="frame">
        <div class="head-row">
          <div class="q-chip">${currentIndex + 1}/${questions.length} | Q${q.qNumber}</div>
          <div class="title-text">${escapeHtml(q.title)} | L4-E3</div>
          <div class="status" id="question-status"></div>
        </div>
        <div class="inst">Drag sentence tokens into the matching Korean fragment. Use original order.</div>
        <div class="land-grid">
          <div class="panel">
            <div class="panel-head">
              <div class="panel-title">KR Fragments</div>
  ...
  if (bank && finalState) {
    ...
    bank.innerHTML = (finalState.chipOrder || [])
      .map((fid) => {
        ...
        if (!frag) return "";
        const solved = isFinalFragSolved(finalState, frag);
        const c = pickFragColor(Number(fid));
        const koOnly = compactWhitespace(frag.ko || "") || compactWhitespace(frag.en || "");
        const picked = getPickedFragIndices(finalState, fid);
        const pickedHtml = picked.length
          ? picked
              .map((idx) => {
                const tok = sentenceTokens[idx] || "";
                const style = buildFinalTokenStyle([Number(fid)]);
                return `<span class="token-pill final-picked-token filled" data-final-pick-fid="${fid}" data-final-token-idx="${idx}" style="${style}">${escapeHtml(tok)}</span>`;
              })
              .join(" ")
          : `<span class="final-picked-empty">Drop sentence tokens here</span>`;

        return `
          <div class="frag-chip final-chip${solved ? " completed" : ""}" data-final-frag-id="${fid}" style="--frag-color:${c.solid}; --frag-soft:${c.soft}; --frag-border:${c.solid};" title="${escapeHtml(frag.en)}">
            <div class="frag-ko">${escapeHtml(koOnly)}</div>
            <div class="final-frag-picked">${pickedHtml}</div>
          </div>
        `;
      })
      .join("");
  ...
  if (sentence && finalState) {
    ...
    sentence.innerHTML = sentenceTokens
      .map((tok, idx) => {
        ...
        const style = buildFinalSentenceTokenStyle(owners);
        const tokenFull = isFinalTokenFullySolved(q, finalState, idx);
        const cls = ["token-pill", "in-slot", "merged-slot-pill", "final-slot", "final-source-token"];
        if (owners.length) cls.push("filled");
        if (tokenFull) cls.push("final-slot-complete");
        return `<span class="${cls.join(" ")}" data-final-slot-idx="${idx}" draggable="${isLocked ? "false" : "true"}" style="${style}">${escapeHtml(tok)}</span>`;
      })
      .join(" ");
  }

  const totalFrags = Array.isArray(q.fragments) ? q.fragments.length : 0;
  const solvedCount = countSolvedFinalFrags(q, finalState);
  if (leftStatus) leftStatus.textContent = `${solvedCount}/${totalFrags}`;
  if (rightStatus) rightStatus.textContent = `Solved ${solvedCount}/${totalFrags}`;
  if (hint) hint.textContent = "Drag sentence tokens into Korean fragments. Click a token inside a fragment to remove.";
  if (topStatus) topStatus.textContent = `Progress ${solvedCount}/${totalFrags}`;
  if (submitBtn) submitBtn.disabled = isLocked;
  if (nextBtn) nextBtn.disabled = !isLocked;
  wireStageEvents();
}
...
function wireStageEvents() {
  ...
  if (finalBank && finalBank.dataset.bound !== "1") {
    ...
    finalBank.addEventListener("dragover", (ev) => {
      if (stageMode !== "final" || isLocked) return;
      const chip = ev.target?.closest?.(".final-chip[data-final-frag-id]");
      if (!chip) return;
      ev.preventDefault();
    ...
    finalBank.addEventListener("dragleave", (ev) => {
      const chip = ev.target?.closest?.(".final-chip[data-final-frag-id]");
      if (!chip) return;
      if (!chip.contains(ev.relatedTarget)) chip.classList.remove("drop-ready");
    });

    finalBank.addEventListener("drop", (ev) => {
      if (stageMode !== "final" || isLocked) return;
      const chip = ev.target?.closest?.(".final-chip[data-final-frag-id]");
      if (!chip) return;
      ev.preventDefault();
      ...
      const tokenIdx = Number.isInteger(Number(fromTransfer)) ? Number(fromTransfer) : Number(finalDragTokenIdx);
      if (!Number.isInteger(fid) || !Number.isInteger(tokenIdx)) return;
      ...
      if (!cq || !cs || !fs) return;
      if (!frag) return;
      const wasSolved = isFinalFragSolved(fs, frag);
      if (!fs.pickedIdxByFragId || typeof fs.pickedIdxByFragId !== "object") fs.pickedIdxByFragId = {};
      const list = Array.isArray(fs.pickedIdxByFragId[fid]) ? fs.pickedIdxByFragId[fid].slice() : [];
      if (!list.includes(tokenIdx)) list.push(tokenIdx);
      fs.pickedIdxByFragId[fid] = Array.from(new Set(list)).sort((a, b) => a - b);
      finalDragTokenIdx = null;
      const nowSolved = isFinalFragSolved(fs, frag);
      renderStage();
      if (!wasSolved && nowSolved) {
        showToast("ok", "Fragment complete.");
      }
    });

    finalBank.addEventListener("click", (ev) => {
      if (stageMode !== "final" || isLocked) return;
      const pick = ev.target?.closest?.("[data-final-pick-fid][data-final-token-idx]");
      if (!pick) return;
      const fid = Number(pick.getAttribute("data-final-pick-fid"));
      const tokenIdx = Number(pick.getAttribute("data-final-token-idx"));
      if (!Number.isInteger(fid) || !Number.isInteger(tokenIdx)) return;
      const cq = getCurrentQuestion();
      const cs = getQuestionState(cq);
      const fs = getFinalArrangeState(cq, cs);
      if (!cq || !cs || !fs) return;
      if (!fs.pickedIdxByFragId || typeof fs.pickedIdxByFragId !== "object") fs.pickedIdxByFragId = {};
      const list = Array.isArray(fs.pickedIdxByFragId[fid]) ? fs.pickedIdxByFragId[fid] : [];
      fs.pickedIdxByFragId[fid] = list.filter((n) => Number(n) !== tokenIdx);
      renderStage();
    });
  }
  ...
  if (finalSentence && finalSentence.dataset.bound !== "1") {
    ...
    finalSentence.addEventListener("dragstart", (ev) => {
      if (stageMode !== "final" || isLocked) return;
      const slot = ev.target?.closest?.("[data-final-slot-idx]");
      if (!slot) return;
      const tokenIdx = Number(slot.getAttribute("data-final-slot-idx"));
      if (!Number.isInteger(tokenIdx)) return;
      finalDragTokenIdx = tokenIdx;
      slot.classList.add("dragging");
      if (ev.dataTransfer) {
        ev.dataTransfer.effectAllowed = "copy";
        ev.dataTransfer.setData("text/plain", String(tokenIdx));
      }
    });

    finalSentence.addEventListener("dragend", (ev) => {
      const slot = ev.target?.closest?.("[data-final-slot-idx]");
      if (slot) slot.classList.remove("dragging");
      finalDragTokenIdx = null;
      document.querySelectorAll(".final-chip.drop-ready").forEach((el) => el.classList.remove("drop-ready"));
    });
  }
}
...
function submitCurrent() {
  ...
  const finalState = getFinalArrangeState(q, state);
  const solvedCount = countSolvedFinalFrags(q, finalState);
  if (solvedCount < totalFrags) {
    upsertResult({
      no: currentIndex + 1,
      qNumber: q.qNumber,
      word: `Pleks L4-E3 / Q${q.qNumber}`,
      question: buildQuestionLog(q),
      selected: buildSelectedLog(q),
      correct: false,
      modelAnswer: q.answer,
    });
    showToast("no", "Check fragment-token matches.");
    return;
  }
...
function buildSelectedLog(q) {
  ...
  const picked = Object.entries(state?.finalArrange?.pickedIdxByFragId || {})
    .map(([k, v]) => {
      const fid = Number(k);
      const arr = Array.isArray(v) ? v.slice().sort((a, b) => Number(a) - Number(b)) : [];
      if (!Number.isInteger(fid) || !arr.length) return "";
      return `${fid}:${arr.join(".")}`;
    })
    .filter(Boolean)
    .join(",");
  return `clusters:${clusterLog || "-"} || merged:${merged || "-"} || picked:${picked || "-"}`;
}
