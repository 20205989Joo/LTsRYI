(function () {
  ...
  var alphaState = {
    ...
    round2TargetPath: "pleks-l1e2.html?round2=1&round2Script=pleks-l1e2_round2.js",
    round2JumpCount: 0,
    lastRound2Url: ""
  };
  ...
  function readTourStateSafe() {
    try {
      ...
      var raw = sessionStorage.getItem("PleksFullTourState");
      if (!raw) return null;
  ...
  function writeTourQCapToState(nextQCap) {
    ...
    if (saved && typeof saved === "object") {
      ...
      try {
        sessionStorage.setItem("PleksFullTourState", JSON.stringify(saved));
      } catch (_) {}
    }
  ...
  function getSubmitBtn() {
    return byId("submit-btn") || byId("btn-submit");
  }
  ...
  function getRound2TargetPath() {
    ...
    return String(alphaState.round2TargetPath || "pleks-l1e2.html?round2=1&round2Script=pleks-l1e2_round2.js");
  }
  ...
  function getCurrentFlowStateSafe(baseKey) {
    ...
    if (!key) return null;
    var map = readStorageJsonObjectSafe("PleksRound2FlowMap");
    var entry = map[key];
  ...
  function clearRound2FlowAndQuizState() {
    ...
    if (baseKey) {
      clearMapEntrySafe("PleksRound2FlowMap", baseKey);
    }
  ...
  function renderPanelControlsState() {
    ...
    if (hud) hud.style.display = alphaState.panelControlsVisible ? "block" : "none";
    if (btn) {
  ...
  function applyTourQCap(nextQCap) {
    ...
    try {
      if (window.PleksRound2LifeUI && typeof window.PleksRound2LifeUI.sync === "function") {
        setTimeout(function () { window.PleksRound2LifeUI.sync(); }, 0);
      }
    } catch (_) {}
  ...
  function pickActiveRound2ModuleInfo(requireMounted) {
    ...
    try {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        if (!/^PleksL\d+E\d+Round2$/.test(key)) continue;
  ...
  function resolveSubmitHandler() {
    var route = getRound2RouteMeta();
    if (route.isRound2) syncRound2GlobalAliases();
    if (typeof window.submitAnswer === "function") return window.submitAnswer;
    if (typeof window.submitCurrent === "function") return window.submitCurrent;
    var mod = pickActiveRound2Module(route.isRound2);
    if (mod && typeof mod.submitAnswer === "function") return mod.submitAnswer;
    return null;
  }
  ...
  function forceSubmit() {
    var submitHandler = resolveSubmitHandler();
  ...
  function ensureDebugPanel() {
    ...
    if (!panel && document.body) {
      ...
      submitButton.id = "alpha-submit-btn";
      submitButton.type = "button";
      submitButton.textContent = "ALPHA SUBMIT";
      submitButton.style.background = "#17181c";
      submitButton.style.color = "#f4f4f4";
      submitButton.style.border = "1px solid #3a3f48";
      submitButton.style.borderRadius = "10px";
      submitButton.style.padding = "8px 10px";
      submitButton.style.font = "700 12px/1 sans-serif";
      submitButton.style.letterSpacing = "0.35px";
      submitButton.style.cursor = "pointer";
      submitButton.style.boxShadow = "0 6px 16px rgba(0,0,0,0.25)";
      ...
      controlsWrap.appendChild(submitButton);
  ...
  function start() {
    ...
    window.alphaForceSubmit = forceSubmit;
