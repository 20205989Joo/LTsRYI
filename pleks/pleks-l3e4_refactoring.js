...
const STRICT_DATA_MODE = true;

let subcategory = "Grammar";
let level = "Basic";
let day = "310";
let quizTitle = "quiz_Grammar_Basic_310";
let userId = "";
...
const INDEX_KEYS = ["ChunkIndexList", "Index", "ChunkIndex", "ChunkIndices", "Indices"];
const LASSO_KEYS = ["LassoIndexList", "LassoIndex", "LassoIndices", "ScopeIndex", "ScopeIndices", "ChunkScopeIndex"];
...
function renderZoomOverlay() {
  ...
  if (zoomState.dragging) {
    const stroke = computeScopeSelectionIndices(ch, zoomState.dragCurrentIdx);
    scopeIndices = mergeIndexSets(scopeIndices, stroke);
  }
...
function wireZoomInteractions() {
  ...
  function finalizePointer(ev) {
    ...
    let stroke = [];
    if (ch) {
      stroke = computeScopeSelectionIndices(ch, zoomState.dragCurrentIdx);
    }
    ...
    const prevScope = Array.isArray(ch.scopeIndices) ? ch.scopeIndices : [];

    if (expected.length) {
      if (!isSubsetIndexSet(stroke, expected)) {
        showToast("no", "Scope mismatch. Match the exact '~' span.");
        renderZoomOverlay();
        return;
      }

      const merged = mergeIndexSets(prevScope, stroke);
      ch.scopeIndices = merged;
      if (!isSameIndexSet(merged, expected)) {
        ch.stage2Done = false;
        showToast("ok", "Scope added. Keep drawing.");
        renderZoomOverlay();
        return;
      }
    } else {
      ch.scopeIndices = mergeIndexSets(prevScope, stroke);
    }
...
function buildLessonMeta() {
  ...
  lessonTitle = String(row?.["LessonTitle"] ?? row?.["Title"] ?? "").trim();
  exerciseTitle = String(row?.["Title"] ?? "").trim();
}

function buildQuestionsFromRows() {
  const hasOneSheetSchema = rawRows.some(
    (r) =>
      String(r?.["SentenceEN"] ?? "").trim() &&
      String(r?.["ChunkENList"] ?? "").trim() &&
      String(r?.["UnitTokenPlan"] ?? "").trim()
  );
  if (!hasOneSheetSchema) {
    throw new Error("One-sheet schema columns are missing. Expected SentenceEN/ChunkENList/UnitTokenPlan.");
  }
  ...
  questions = filtered.map((r, idx) => {
    const parsed = parseOneSheetQuestionRow(r);
    ...
    const configuredMidKoUnits = [];
    const configuredMidEnUnits = [];
    const configuredMidUnitData = parseOneSheetMidstageUnitData(r);
    const configuredMegaUnitData = parseOneSheetMegastageUnitData(r, configuredMidUnitData);
    ...
    const chunks = (parsed.chunks || []).map((ch, ci) => {
      ...
      const expectedScopeIndices = STRICT_DATA_MODE
        ? fromLassoColumn
        : (fromLassoColumn.length ? fromLassoColumn : fromMidUnitData);

      if (STRICT_DATA_MODE) {
        if (!fromIndexColumn.length) strictRowErrors.push(`chunk#${ci + 1}: missing ChunkIndexList`);
        if (!fromLassoColumn.length) strictRowErrors.push(`chunk#${ci + 1}: missing LassoIndexList`);
      }
      ...
      return {
        ...
        mapSource: fromIndexColumn.length ? "index" : (STRICT_DATA_MODE ? "missing" : "auto"),
        scopeSource: fromLassoColumn.length ? "lasso" : (STRICT_DATA_MODE ? "missing" : (fromMidUnitData.length ? "middata" : "none")),
      };
    });

    const rawAnswerKorean = String(r["AnswerKO"] ?? r["Answer"] ?? "").trim();
    const megaAnswerKorean = buildJoinedMegaKorean(configuredMegaUnitData);
    const answerKorean = resolveCanonicalAnswerKorean(
      rawAnswerKorean,
      megaAnswerKorean,
      Number(r["QNumber"]) || idx + 1
    );
    if (STRICT_DATA_MODE) {
      const strictErrors = validateStrictOneSheetQuestionData({
        row: r,
        qNumber: Number(r["QNumber"]) || idx + 1,
        parsed,
        configuredMidUnitData,
        configuredMegaUnitData,
        answerKorean: rawAnswerKorean,
        megaAnswerKorean,
        strictRowErrors,
      });
      if (strictErrors.length) {
        throw new Error(`[StrictData] Q${Number(r["QNumber"]) || idx + 1}: ${strictErrors.join(" | ")}`);
      }
    }
...
function parseOneSheetQuestionRow(row) {
  const sentence = compactWhitespace(String(row?.["SentenceEN"] ?? "").trim());
  const chunkEn = parsePipeListCell(row?.["ChunkENList"]);
  const chunkKo = parsePipeListCell(row?.["ChunkKOList"]);
  const chunks = chunkEn.map((base, i) => ({
    id: i,
    base,
    meaning: compactWhitespace(chunkKo[i] || ""),
  }));
...
function parseOneSheetIndexRanges(raw) {
  const parts = parsePipeListCell(raw);
  return parts.map((part) => {
    const idx = parseIndexGroup(part);
    if (!idx.length) return { startIdx: null, endIdx: null };
    return { startIdx: Math.min(...idx), endIdx: Math.max(...idx) };
  });
}

function parseOneSheetUnitTokenPlan(raw) {
  const units = [];
  const unitParts = parsePipeListCell(raw);
  unitParts.forEach((part) => {
    const noPrefix = String(part || "").replace(/^\s*u\d+\s*:\s*/i, "").trim();
    const tokenParts = noPrefix
      .split(/\s*\|\s*/)
      .map((x) => x.trim())
      .filter(Boolean);

    const tokens = tokenParts.map((tok, i) => parseOneSheetUnitToken(tok, i)).filter(Boolean);
    units.push(tokens);
  });
  return units;
}

function parseOneSheetUnitToken(raw, fallbackId) {
  const mm = String(raw || "").match(/^([^:]+)::(.+)$/);
  if (!mm) return null;

  const head = String(mm[1] || "").trim();
  const body = String(mm[2] || "").trim();
  ...
  if (at > -1) {
    ...
    idxRaw = body.slice(at + 1).trim();
  }

  const hm = head.match(/^([a-zA-Z]+)(?:#c(\d+))?$/);
  const kind = String(hm?.[1] || "plain").toLowerCase();
  const c = Number(hm?.[2]);
  const chunkId = Number.isInteger(c) && c > 0 ? c - 1 : null;
  const enIndices = idxRaw ? parseIndexGroup(idxRaw) : [];

  return {
    id: Number.isInteger(fallbackId) ? fallbackId : 0,
    kind,
...
function parseOneSheetMidstageUnitData(row) {
  const enUnits = parsePipeListCell(row?.["UnitENList"]);
  const koUnits = parsePipeListCell(row?.["UnitKOList"]);
  const ranges = parseOneSheetIndexRanges(row?.["UnitIndexList"]);
  const tokenPlan = parseOneSheetUnitTokenPlan(row?.["UnitTokenPlan"]);

  const maxLen = Math.max(enUnits.length, koUnits.length, ranges.length, tokenPlan.length);
  const out = [];
  for (let i = 0; i < maxLen; i += 1) {
    const label = compactWhitespace(enUnits[i] || "");
    const text = compactWhitespace(koUnits[i] || "");
    const unitRange = ranges[i] || { startIdx: null, endIdx: null };
    let tokens = Array.isArray(tokenPlan[i]) ? tokenPlan[i].map((x) => ({ ...x })) : [];

    if (!tokens.length && text) {
      const enIndices =
        Number.isInteger(unitRange.startIdx) &&
        Number.isInteger(unitRange.endIdx) &&
        unitRange.endIdx >= unitRange.startIdx
          ? range(unitRange.startIdx, unitRange.endIdx)
          : [];
      tokens = [{ id: 0, kind: "plain", text, chunkId: null, enIndices }];
    }

    tokens = tokens
      .map((tok, ti) => {
        const kind = compactWhitespace(String(tok?.kind || "plain").toLowerCase()) || "plain";
        const tText = compactWhitespace(tok?.text || "");
        if (!tText) return null;
        const rawChunkId = tok?.chunkId;
        const parsedChunkId = Number(rawChunkId);
        const chunkId =
          (Number.isInteger(rawChunkId) && rawChunkId >= 0)
          || (typeof rawChunkId === "string" && rawChunkId.trim() !== "" && Number.isInteger(parsedChunkId) && parsedChunkId >= 0)
            ? parsedChunkId
            : null;
        let enIndices = Array.isArray(tok?.enIndices)
        ...
        if (
          ...
          enIndices = range(unitRange.startIdx, unitRange.endIdx);
        }
        return {
          id: Number.isInteger(tok?.id) ? Number(tok.id) : ti,
          kind,
          text: tText,
          chunkId,
          enIndices,
    ...
    if (!label && !text && !tokens.length) continue;

    const chunkIds = Array.from(
      new Set(tokens.map((t) => Number(t?.chunkId)).filter((x) => Number.isInteger(x) && x >= 0))
    );
    ...
    out.push({
      ...
      label,
      text,
      chunkId: chunkIds.length === 1 ? chunkIds[0] : null,
      startIdx: Number.isInteger(unitRange.startIdx) ? Number(unitRange.startIdx) : null,
      endIdx: Number.isInteger(unitRange.endIdx) ? Number(unitRange.endIdx) : null,
      tokens,
      done: false,
    });
  }
...
function parseOneSheetMegastageUnitData(row, sourceMidUnits) {
  const enUnits = parsePipeListCell(row?.["MegaENList"]);
  const koUnits = parsePipeListCell(row?.["MegaKOList"]);
  const ranges = parseOneSheetIndexRanges(row?.["MegaIndexList"]);
  const members = parsePipeListCell(row?.["MegaMembers"]).map((part) => {
    const body = String(part || "").replace(/^\s*m\d+\s*:\s*/i, "").trim();
    if (!body) return [];
    return body
      .split("+")
  ...
  const maxLen = Math.max(enUnits.length, koUnits.length, ranges.length, members.length);
  const mids = Array.isArray(sourceMidUnits) ? sourceMidUnits : [];
  const out = [];

  for (let i = 0; i < maxLen; i += 1) {
    const unitIds = Array.isArray(members[i]) ? members[i].slice() : [];
    const linked = unitIds
      .map((uid) => mids.find((u) => Number(u?.id) === uid))
      .filter(Boolean);

    const label = compactWhitespace(enUnits[i] || linked.map((u) => compactWhitespace(u?.label || "")).join(" "));
    const text = compactWhitespace(koUnits[i] || linked.map((u) => compactWhitespace(u?.text || "")).join(" "));
    if (!label && !text && !unitIds.length) continue;

    const rangeCell = ranges[i] || { startIdx: null, endIdx: null };
    const starts = linked.map((u) => Number(u?.startIdx)).filter((x) => Number.isInteger(x));
    const ends = linked.map((u) => Number(u?.endIdx)).filter((x) => Number.isInteger(x));

    const startIdx = Number.isInteger(rangeCell.startIdx)
      ? Number(rangeCell.startIdx)
      : (starts.length ? Math.min(...starts) : null);
    const endIdx = Number.isInteger(rangeCell.endIdx)
      ? Number(rangeCell.endIdx)
      : (ends.length ? Math.max(...ends) : null);

    const tokens = linked.map((u, ti) => ({
      id: ti,
...
function buildJoinedMegaKorean(megaList) {
  const list = Array.isArray(megaList) ? megaList : [];
  if (!list.length) return "";
  const parts = list
    .map((u) => compactWhitespace(u?.text || ""))
    .filter(Boolean);
  if (!parts.length) return "";
  if (parts.some((t) => isLikelyGarbledText(t))) return "";
  return compactWhitespace(parts.join(" "));
}

function resolveCanonicalAnswerKorean(rawAnswer, megaAnswer, qNumber) {
  const base = compactWhitespace(rawAnswer || "");
  const mega = compactWhitespace(megaAnswer || "");
  if (!mega) return base;

  const baseNorm = normalizeKoreanForCompare(base);
  const megaNorm = normalizeKoreanForCompare(mega);
  if (baseNorm && megaNorm && baseNorm !== megaNorm) {
    console.warn(
      `[Pleks L3-E4] Q${qNumber}: AnswerKO differs from MegaKOList. Using MegaKOList for grading.`
    );
  }
  return mega;
}

function validateStrictOneSheetQuestionData(input) {
  const row = input?.row || {};
  const qNumber = Number(input?.qNumber);
  const parsed = input?.parsed || {};
  const mids = Array.isArray(input?.configuredMidUnitData) ? input.configuredMidUnitData : [];
  const megas = Array.isArray(input?.configuredMegaUnitData) ? input.configuredMegaUnitData : [];
  const strictRowErrors = Array.isArray(input?.strictRowErrors) ? input.strictRowErrors : [];
  const answerKorean = compactWhitespace(input?.answerKorean || "");
  const megaAnswerKorean = compactWhitespace(input?.megaAnswerKorean || "");

  const errs = [];
  if (!compactWhitespace(parsed?.sentence || "")) errs.push("SentenceEN is empty");
  if (!Array.isArray(parsed?.chunks) || !parsed.chunks.length) errs.push("ChunkENList is empty");

  const chunkCount = parsePipeListCell(row?.["ChunkENList"]).length;
  const chunkKoCount = parsePipeListCell(row?.["ChunkKOList"]).length;
  const chunkIdxCount = parsePipeListCell(row?.["ChunkIndexList"]).length;
  const lassoIdxCount = parsePipeListCell(row?.["LassoIndexList"]).length;
  if (!chunkCount) errs.push("ChunkENList has no items");
  if (chunkKoCount !== chunkCount) errs.push(`ChunkKOList count ${chunkKoCount} != ChunkENList count ${chunkCount}`);
  if (chunkIdxCount !== chunkCount) errs.push(`ChunkIndexList count ${chunkIdxCount} != ChunkENList count ${chunkCount}`);
  if (lassoIdxCount !== chunkCount) errs.push(`LassoIndexList count ${lassoIdxCount} != ChunkENList count ${chunkCount}`);
  errs.push(...strictRowErrors);

  const unitEnCount = parsePipeListCell(row?.["UnitENList"]).length;
  const unitKoCount = parsePipeListCell(row?.["UnitKOList"]).length;
  const unitIdxCount = parsePipeListCell(row?.["UnitIndexList"]).length;
  const unitPlan = parseOneSheetUnitTokenPlan(row?.["UnitTokenPlan"]);
  if (!unitEnCount) errs.push("UnitENList has no items");
  if (unitKoCount !== unitEnCount) errs.push(`UnitKOList count ${unitKoCount} != UnitENList count ${unitEnCount}`);
  if (unitIdxCount !== unitEnCount) errs.push(`UnitIndexList count ${unitIdxCount} != UnitENList count ${unitEnCount}`);
  if (unitPlan.length !== unitEnCount) errs.push(`UnitTokenPlan count ${unitPlan.length} != UnitENList count ${unitEnCount}`);
  if (unitPlan.some((x) => !Array.isArray(x) || !x.length)) errs.push("UnitTokenPlan has empty unit token list");

  if (!mids.length) errs.push("configured mid unit data is empty");
  mids.forEach((u, i) => {
    if (!compactWhitespace(u?.text || "")) errs.push(`mid unit#${i + 1} text is empty`);
    if (!Number.isInteger(u?.startIdx) || !Number.isInteger(u?.endIdx) || Number(u.endIdx) < Number(u.startIdx)) {
      errs.push(`mid unit#${i + 1} range is invalid`);
    }
    if (!Array.isArray(u?.tokens) || !u.tokens.length) errs.push(`mid unit#${i + 1} tokens are empty`);
  });

  const megaEnCount = parsePipeListCell(row?.["MegaENList"]).length;
  const megaKoCount = parsePipeListCell(row?.["MegaKOList"]).length;
  const megaIdxCount = parsePipeListCell(row?.["MegaIndexList"]).length;
  const megaMembersCount = parsePipeListCell(row?.["MegaMembers"]).length;
  if (!megaEnCount) errs.push("MegaENList has no items");
  if (megaKoCount !== megaEnCount) errs.push(`MegaKOList count ${megaKoCount} != MegaENList count ${megaEnCount}`);
  if (megaIdxCount !== megaEnCount) errs.push(`MegaIndexList count ${megaIdxCount} != MegaENList count ${megaEnCount}`);
  if (megaMembersCount !== megaEnCount) errs.push(`MegaMembers count ${megaMembersCount} != MegaENList count ${megaEnCount}`);

  if (!megas.length) errs.push("configured mega unit data is empty");
  megas.forEach((m, i) => {
    if (!compactWhitespace(m?.text || "")) errs.push(`mega unit#${i + 1} text is empty`);
    if (!Array.isArray(m?.unitIds) || !m.unitIds.length) errs.push(`mega unit#${i + 1} has no members`);
  });

  if (!answerKorean) errs.push("AnswerKO is empty");
  if (!megaAnswerKorean) errs.push("MegaKOList joined text is empty");
  if (answerKorean && megaAnswerKorean) {
    const a = normalizeKoreanForCompare(answerKorean);
    const b = normalizeKoreanForCompare(megaAnswerKorean);
    if (a !== b) errs.push("AnswerKO != joined MegaKOList");
  }

  if (!Number.isInteger(qNumber) || qNumber <= 0) errs.push("QNumber is invalid");
  return errs;
}
...
function isSubsetIndexSet(subset, superset) {
  const a = mergeIndexSets(subset, []);
  const b = new Set(mergeIndexSets(superset, []));
...
function submitCurrent() {
  ...
  const expectedMegaAnswer = isMegaStageActive() ? buildJoinedMegaKorean(megaUnits) : "";
  const expectedAnswer = compactWhitespace(expectedMegaAnswer || q.answerKorean || "");
  const expectedNorm = normalizeKoreanForCompare(expectedAnswer);
  const correct = normalizeKoreanForCompare(userKorean) === expectedNorm;
