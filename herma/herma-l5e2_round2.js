(function initHermaL5E2Round2(global) {
  "use strict";

  if (!global || global.HermaL5E2Round2) return;

  var MODULE_NAME = "HermaL5E2Round2";
  var SCRIPT_NAME = "herma-l5e2_round2.js";
  var STYLE_ID = "herma-l5e2-round2-style";
  var INSTRUCTION_KR = "\uC6D0\uBB38\uACFC \uBA85\uC0AC\uAD6C\uB97C \uC11C\uB85C \uBC14\uAFD4 \uC368\uBCF4\uC138\uC694.";
  var LABEL_SOURCE_KR = "\uC81C\uC2DC\uBB38";
  var LABEL_ORIGINAL_KR = "\uC6D0\uBB38";
  var LABEL_NP_KR = "\uBA85\uC0AC\uAD6C";
  var LABEL_MEANING_KR = "\uD55C\uAD6D\uC5B4 \uB73B";
  var LABEL_ANSWER_KR = "\uB2F5";
  var INPUT_REQUIRED_MSG = "\uBB38\uC7A5\uC744 \uC785\uB825\uD558\uC138\uC694.";
  ...
  var state = {
    ...
    level: "Basic",
    day: "119",
    quizTitle: "quiz_Grammar_Basic_119_round2",
    rawRows: [],
    questions: [],
    currentIndex: 0,
    results: [],
  ...
  function parseAcceptedAnswers(value) {
    ...
    String(value == null ? "" : value)
      .split(/[|]/)
      .map(function (s) { return normalizeSentence(s); })
      .filter(Boolean)
      .forEach(function (s) {
  ...
  function inferPromptType(round2Type, promptText, modelAnswer) {
    var t = String(round2Type || "").trim().toLowerCase();
    if (t.indexOf("np-to-orig") !== -1 || t.indexOf("n2o") !== -1) return "np";
    if (t.indexOf("orig-to-np") !== -1 || t.indexOf("o2n") !== -1) return "original";

    var prompt = String(promptText || "").trim();
    var model = String(modelAnswer || "").trim();
    var promptLooksSentence = /[.?!]\s*$/.test(prompt);
    var modelLooksSentence = /[.?!]\s*$/.test(model);

    if (!promptLooksSentence && modelLooksSentence) return "np";
    return "original";
  }

  function normalizePromptText(text, promptType) {
    var s = String(text || "").trim();
    if (!s) return "";
    if (promptType === "np") return s.replace(/[.?!]\s*$/, "");
    return ensureEndMark(s);
  }
  ...
  function shouldPrefillToken(token) {
    var t = normalizeToken(token);
    return t === "the" || t === "a" || t === "an" ||
      t === "that" || t === "which" || t === "who" || t === "whom" ||
      t === "whose" || t === "where" || t === "when" || t === "why" ||
      t === "was" || t === "were";
  }
  ...
  function buildAnswerBlueprint(answerText) {
    ...
    rawTokens.forEach(function (raw) {
      ...
      if (token) {
        var fixed = shouldPrefillToken(token, wordCursor);
        parts.push({ kind: "word", value: token, fixed: fixed });
        if (!fixed) {
  ...
  function buildSlotInputHTML(index, expectedToken) {
    var width = clamp((String(expectedToken || "").length * 8) + 8, 20, 92);
    var expectedAttr = String(expectedToken == null ? "" : expectedToken).replace(/"/g, "&quot;");
    return '<input class="r51-slot-input" id="r51-slot-' + index + '" data-idx="' + index + '" data-expected="' + expectedAttr + '" style="width:' + width + 'px;" autocomplete="off" />';
  }

  function getSlotElements() {
    return Array.from(document.querySelectorAll(".r51-slot-input"));
  }
  ...
  function renderSlotArea(q) {
    ...
    var html = q.answer.parts.map(function (part) {
      if (part.kind === "word") {
        if (part.fixed) {
          return '<span class="r51-fixed-token">' + escapeHtml(part.value) + "</span>";
        }
        var input = buildSlotInputHTML(inputIdx, part.value);
        inputIdx += 1;
        return input;
      }
      return '<span class="r51-fixed-punc">' + escapeHtml(part.value) + "</span>";
    }).join("");

    return [
      '<div class="r51-slot-area" id="r51-slot-area">',
      html,
      '  <span class="r51-dot-tail">.</span>',
      "</div>"
    ].join("\n");
  }
  ...
  function ensureStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = [
      ".r51-sheet-box{background:#fff;border:2px solid #101010;border-radius:8px;padding:10px;margin-bottom:12px;}",
      ".r51-sheet-head{font-size:12px;font-weight:900;letter-spacing:0.06em;color:#101010;text-transform:uppercase;margin-bottom:6px;}",
      ".r51-q-label{font-weight:900;font-size:16px;margin-bottom:10px;color:#111;}",
      ".r51-instruction{font-size:17px;font-weight:900;color:#111;margin-bottom:10px;}",
      ".r51-stem-box{border:1px solid #b8b8b8;border-radius:4px;padding:8px 10px;margin:8px 0 6px;background:#fff;}",
      ".r51-stem{font-size:14px;line-height:1.7;color:#101010;margin:10px 0;word-break:keep-all;}",
      ".r51-keyword{display:inline;color:#111;font-weight:900;font-style:italic;}",
      ".r51-answer-wrap{display:flex;align-items:flex-end;gap:8px;margin-top:18px;}",
      ".r51-answer-label{font-size:13px;font-weight:900;color:#161616;white-space:nowrap;}",
      ".r51-slot-area{flex:1;display:flex;align-items:flex-end;flex-wrap:wrap;gap:3px;padding:2px 0 0;min-height:44px;}",
      ".r51-fixed-token{display:inline-block;line-height:22px;padding:0 1px;font-weight:900;font-size:13px;color:#111;border-bottom:2px solid #111;}",
      ".r51-slot-input{width:48px;min-width:20px;height:22px;border:none;border-bottom:2px solid #111;border-radius:0;padding:0 1px 1px;font-size:13px;line-height:20px;text-align:center;outline:none;background:transparent;color:#111;}",
      ".r51-slot-input:focus{border-bottom-width:3px;}",
      ".r51-fixed-punc{display:inline-block;line-height:22px;padding:0 1px;font-weight:900;font-size:13px;color:#111;}",
      ".r51-dot-tail{font-weight:900;font-size:16px;color:#111;line-height:20px;padding:0 1px 0 0;}",
      ".r51-btn-row{display:flex;gap:10px;margin-top:12px;}",
      ".r51-quiz-btn{flex:1;margin-top:0;padding:7px 12px;font-size:13px;background:#1f1f1f;color:#fff;border:1px solid #0a0a0a;border-radius:6px;cursor:pointer;font-weight:900;}",
      ".r51-quiz-btn:disabled{opacity:0.5;cursor:not-allowed;}",
      ".r51-feedback{margin-top:7px;font-weight:900;font-size:12px;min-height:15px;}",
      ".r51-feedback.ok{color:#1d6d20;}",
      ".r51-feedback.bad{color:#a32323;}"
    ].join("");
  ...
  function buildQuestionsFromRows() {
    ...
    state.questions = filtered.map(function (r, idx) {
      ...
      var promptType = inferPromptType(round2Type, rawPrompt, modelAnswer);
      var promptText = normalizePromptText(rawPrompt, promptType);
      var blueprint = buildAnswerBlueprint(modelAnswer);
      var accepted = parseAcceptedAnswers(r["AcceptedAnswers"]);
      var modelNorm = normalizeSentence(modelAnswer);
      if (modelNorm && accepted.indexOf(modelNorm) === -1) accepted.push(modelNorm);

      return {
        no: idx + 1,
        qNumber: Number(r["QNumber"]) || idx + 1,
        title: String(r["Title"] || "").trim() || "Herma L5-E2 Round2",
        ...
        answer: {
          ...
          inputWords: blueprint.inputWords,
          inputNorm: blueprint.inputNorm,
        },
        acceptedNorm: accepted,
        active: active,
      };
    }).filter(function (q) {
      return !!q.active && !!q.promptText && !!q.modelAnswer && q.answer.inputWords.length > 0;
    });
  }
  ...
  function renderIntro() {
    ...
    area.innerHTML = [
      '<div class="r51-sheet-box">',
      '  <div class="r51-sheet-head">Round2 Test</div>',
      '  <div class="r51-instruction">' + escapeHtml(instruction) + "</div>",
      '  <div style="font-size:12px;line-height:1.5;color:#2b2b2b;margin-bottom:8px;">',
      "    Total Questions: " + total,
      "  </div>",
      '  <button class="r51-quiz-btn" style="width:100%; margin-top:12px;" onclick="HermaL5E2Round2.startQuiz()">Start</button>',
      "</div>"
    ].join("\n");
  }
  ...
  function renderQuestion() {
    ...
    var promptTag = q.promptType === "np" ? LABEL_NP_KR : LABEL_ORIGINAL_KR;
    var promptLabel = LABEL_SOURCE_KR + " (" + promptTag + ")";

    area.innerHTML = [
      '<div class="r51-q-label">Q. ' + (state.currentIndex + 1) + " / " + state.questions.length + "</div>",
      '<div class="r51-sheet-box">',
      '  <div class="r51-instruction">' + escapeHtml(q.instruction || INSTRUCTION_KR) + "</div>",
      '  <div class="r51-stem-box">',
      '    <div class="r51-stem">' + escapeHtml(promptLabel) + " : " + escapeHtml(q.promptText) + "</div>",
      '    <div class="r51-stem">' + escapeHtml(LABEL_MEANING_KR) + " : " + escapeHtml(q.koreanMeaning || MEANING_FALLBACK_KR) + "</div>",
      "  </div>",
      '  <div class="r51-answer-wrap">',
      '    <span class="r51-answer-label">' + escapeHtml(LABEL_ANSWER_KR) + " :</span>",
      renderSlotArea(q),
      "  </div>",
      '  <div class="r51-feedback" id="feedback-line"></div>',
      '  <div class="r51-btn-row">',
      '    <button class="r51-quiz-btn" id="submit-btn" onclick="HermaL5E2Round2.submitAnswer()">\uC81C\uCD9C</button>',
      '    <button class="r51-quiz-btn" id="next-btn" onclick="HermaL5E2Round2.goNext()" disabled>\uB2E4\uC74C</button>',
      "  </div>",
      "</div>"
    ].join("\n");
  ...
  function submitAnswer() {
    ...
    if (rawTokens.some(function (t) { return !t; })) {
      if (feedback) {
        feedback.className = "r51-feedback bad";
    ...
    if (!isCorrect && q.acceptedNorm.length) {
      isCorrect = q.acceptedNorm.indexOf(normalizeSentence(selectedSentence)) !== -1;
    }

    state.results.push({
      no: state.currentIndex + 1,
      word: "Herma L5-E2 Round2 / Q" + q.qNumber,
    ...
    if (feedback) {
      feedback.className = isCorrect ? "r51-feedback ok" : "r51-feedback bad";
