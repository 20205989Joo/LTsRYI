(function initHermaL3E3Round2(global) {
  "use strict";

  if (!global || global.HermaL3E3Round2) return;

  var MODULE_NAME = "HermaL3E3Round2";
  var SCRIPT_NAME = "herma-l3e3_round2.js";
  var STYLE_ID = "herma-l3e3-round2-style";
  ...
  var state = {
    ...
    level: "Basic",
    day: "111",
    quizTitle: "quiz_Grammar_Basic_111_round2",
    rawRows: [],
    questions: [],
    currentIndex: 0,
    results: [],
  ...
  function buildHintedAnswerParts(answerBlueprint, qNumber) {
    ...
    var hintMode = Number(qNumber) % 2 === 1 ? "none" : "is-only";

    var isWordIndex = -1;
    if (hintMode === "is-only") {
      var words = Array.isArray(answerBlueprint && answerBlueprint.wordTokens)
        ? answerBlueprint.wordTokens
        : [];
      for (var k = 0; k < words.length; k++) {
        if (normalizeToken(words[k]) === "is") {
          isWordIndex = k;
          break;
        }
      }
    ...
    if (hintMode === "is-only") {
      fixedWordIndexes.add(isWordIndex);
    } else if (hintMode === "first-word" && answerBlueprint.wordTokens.length > 0) {
  ...
  function buildQuestionsFromRows() {
    ...
    state.questions = filtered.map(function (r, idx) {
      ...
      return {
        no: idx + 1,
        qNumber: qNumber,
        title: String(r["Title"] || "").trim() || "Herma L3-E3 Round2",
  ...
  function renderIntro() {
    ...
    area.innerHTML = [
      ...
      "  </div>",
      '  <button class="r32-quiz-btn" style="width:100%; margin-top:12px;" onclick="HermaL3E3Round2.startQuiz()">Start</button>',
      "</div>"
    ].join("\n");
  }
  ...
  function renderQuestion() {
    ...
    area.innerHTML = [
      ...
      '    <button class="r32-quiz-btn" id="submit-btn" onclick="HermaL3E3Round2.submitAnswer()">\uC81C\uCD9C</button>',
      '    <button class="r32-quiz-btn" id="next-btn" onclick="HermaL3E3Round2.goNext()" disabled>\uB2E4\uC74C</button>',
      "  </div>",
      "</div>"
    ].join("\n");
  ...
  function submitAnswer() {
    ...
    state.results.push({
      ...
      word: "Herma L3-E3 Round2 / Q" + q.qNumber,
