// ver1.1_26.02.22
// herma-l4e1.js (L4-E1: 같은 단어, 동·명·형으로 쓰기)
// ------------------------------------------------------------
// ✅ 흐름: A/B 문장(같은 단어 하이라이트) → A/B 해석을 '좌우 이동'하며 조각 순서 맞추기
// ✅ A/B 해석 박스: 화살표로 전환 + 클릭한 조각은 현재 박스(activeBox)에 들어감
// ------------------------------------------------------------
...
let level = "Basic";
let day = "115";
let quizTitle = "quiz_Grammar_Basic_115";
let userId = "";
...
function buildQuestionsFromRows() {
  ...
  questions = filtered.map((r, idx) => {
    ...
    const parsedQ = parseABQuestion(questionRaw);
    const parsedA = parseABAnswer(answerRaw);
    const meanings = parseMeaningChoicesFromMeta(parsedA.meta);

    const targetWord = parsedA.targetWord || guessTargetWord(parsedQ.aEng, parsedQ.bEng);
...
function parseABQuestion(s) {
  ...
  // A. ... (태그)B. ... (태그)
  let aEng = "", bEng = "", aTag = "", bTag = "";
  const re = /A\.\s*([\s\S]*?)\s*\(([^)]+)\)\s*B\.\s*([\s\S]*?)\s*\(([^)]+)\)\s*$/i;
  const m = txt.match(re);
  if (m) {
    aEng = stripTrailingPeriod(m[1]).trim();
    aTag = String(m[2] || "").trim();
    bEng = stripTrailingPeriod(m[3]).trim();
    bTag = String(m[4] || "").trim();
    return { aEng, bEng, aTag, bTag };
  }

  // fallback: A. ... B. ...
  const re2 = /A\.\s*([\s\S]*?)\s*B\.\s*([\s\S]*)/i;
  const m2 = txt.match(re2);
  if (m2) {
    aEng = stripTrailingPeriod(m2[1]).trim();
    bEng = stripTrailingPeriod(m2[2]).trim();
  } else {
    aEng = stripTrailingPeriod(txt).trim();
    bEng = "";
  }
  return { aEng, bEng, aTag: "", bTag: "" };
}

function parseABAnswer(answerRaw) {
  ...
  if (arrowIdx !== -1) {
    left = s.slice(0, arrowIdx).trim();
    meta = s.slice(arrowIdx).trim();
  }

  // target word 추출 (→ word ...)
  let targetWord = "";
  if (meta) {
    const m = meta.match(/→\s*([A-Za-z-]+)/);
    if (m) targetWord = String(m[1] || "").trim();
  }

  // A/B 한국어 분리
  let aKor = "", bKor = "";
  const m2 = left.match(/A\.\s*([\s\S]*?)\s*B\.\s*([\s\S]*)/i);
  if (m2) {
    aKor = String(m2[1] || "").trim();
    bKor = String(m2[2] || "").trim();
  } else {
    // fallback: 줄바꿈/구분자 기반
    const lines = left.split(/\n+/).map((x) => x.trim()).filter(Boolean);
    if (lines.length >= 2) {
      aKor = lines[0].replace(/^A\.\s*/i, "").trim();
      bKor = lines[1].replace(/^B\.\s*/i, "").trim();
    } else {
      aKor = left.replace(/^A\.\s*/i, "").trim();
      bKor = "";
    }
  }

  aKor = stripTrailingPeriod(aKor);
  bKor = stripTrailingPeriod(bKor);

  return { aKor, bKor, targetWord, meta };
}

function parseMeaningChoicesFromMeta(meta) {
  const raw = String(meta || "").trim();
  if (!raw) return { m1: "", m2: "" };

  const rhs = raw.replace(/^→\s*/, "").trim();
  const parts = rhs.split(/\s*\/\s*/).map((x) => String(x || "").trim()).filter(Boolean);

  const pickMeaning = (part) => {
    const p = String(part || "").trim();
    if (!p) return "";
    const idx = p.lastIndexOf(":");
    if (idx === -1) return stripTrailingPeriod(p);
    return stripTrailingPeriod(p.slice(idx + 1).trim());
  };

  let m1 = pickMeaning(parts[0] || "");
  let m2 = pickMeaning(parts[1] || "");

  if ((!m1 || !m2) && /\(1\)|\(2\)/.test(rhs)) {
    const m = rhs.match(/\(1\)\s*([\s\S]*?)\s*\(2\)\s*([\s\S]*)$/);
    if (m) {
      m1 = m1 || stripTrailingPeriod(String(m[1] || "").trim());
      m2 = m2 || stripTrailingPeriod(String(m[2] || "").trim());
    }
  }
...
function renderIntro() {
  ...
  const title = questions[0]?.title || "Herma L4-E1";
  const instruction = questions[0]?.instruction || "A/B 문장에서 같은 단어를 품사에 맞게 구분해 자연스럽게 해석하세요.";

  area.innerHTML = `
    <div class="box">
      <div style="font-size:18px; font-weight:900; color:#7e3106; margin-bottom:10px;">📘 Herma L4-E1</div>
...
function submitAnswer() {
  if (stage === "meaning") {
    ...
    stage1Solved = true;
    isAnswered = true; // stage transition 전 잠금
    stopDragTipArcHint();
