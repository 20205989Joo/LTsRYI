...
let level = "Basic";
let day = "122";
let quizTitle = "quiz_Grammar_Basic_122";
let userId = "";
...
function splitEnglishAndKoreanByDash(answerRaw) {
  ...
  const mPuncDash = s.match(/[.?!]\s*[–—]\s*/);
  if (mPuncDash) {
    const idx = mPuncDash.index + 1;
    const english = stripTrailingPunct(s.slice(0, idx).trim());
    const korean = s.slice(mPuncDash.index + mPuncDash[0].length).trim();
    return { english, korean };
  }
  const mDash = s.match(/\s[–—]\s/);
  if (mDash) {
    const idx = mDash.index;
    const english = stripTrailingPunct(s.slice(0, idx).trim());
    const korean = s.slice(idx + mDash[0].length).trim();
    return { english, korean };
  }
  ...
  const firstHangul = s.search(/[가-힣]/);
  if (firstHangul > 0) {
    const pre = s.slice(0, firstHangul).replace(/[–—-]\s*$/g, "").trim();
    const post = s.slice(firstHangul).trim();
    return { english: stripTrailingPunct(pre), korean: post };
  }
...
function extractTwoNominalsFromAnswer(englishAnswer, cRaw){
  ...
  if (!c) {
    const g = guessSubjectByVerb(ans);
    return { subjectA: g, subjectB:"", subjectCombined: g };
  }

  const combined = extractInsertionByIt(ans, c) || guessSubjectByVerb(ans);
  const split = splitCombinedIntoTwoNominalsSmart(combined);
  return {
    subjectA: split.aPhrase,
    subjectB: split.bPhrase,
    subjectCombined: combined.trim()
  };
}

function extractInsertionByIt(ans, c){
  const ansTokens = wordsOnly(ans);
  const cTokens = wordsOnly(c);
  const itIdx = cTokens.map(normalizeWord).indexOf("it");
  if (itIdx === -1) return "";

  const left = cTokens.slice(0, itIdx);
  const right = cTokens.slice(itIdx + 1);
  const ansNorm = ansTokens.map(normalizeWord);

  let start = 0;
  if (left.length) {
    const leftNorm = left.map(normalizeWord).filter(Boolean);
    let lpos = -1;
    let usedLen = 0;
    for (let len = leftNorm.length; len >= 1; len--) {
      const cand = leftNorm.slice(leftNorm.length - len);
      const p = findSubsequenceLast(ansNorm, cand);
      if (p !== -1) {
        lpos = p;
        usedLen = len;
        break;
  ...
  let end = ansTokens.length;
  if (right.length) {
    const rightNorm = right.map(normalizeWord).filter(Boolean);
    let rpos = -1;
    for (let len = rightNorm.length; len >= 1; len--) {
      const cand = rightNorm.slice(0, len);
      const p = findSubsequence(ansNorm.slice(start), cand);
      if (p !== -1) {
        rpos = p;
        break;
      }
    ...
    if (rpos === -1) {
      for (let len = rightNorm.length; len >= 1; len--) {
        const cand = rightNorm.slice(rightNorm.length - len);
        const p = findSubsequence(ansNorm.slice(start), cand);
        if (p !== -1) {
          rpos = p;
          break;
    ...
    if (rpos !== -1) end = start + rpos;
  }

  return ansTokens.slice(start, end).join(" ").trim();
}

function splitCombinedIntoTwoNominalsSmart(combined){
  const toks = wordsOnly(String(combined || "").trim());
  const andIdx = toks.map(normalizeWord).indexOf("and");
  if (andIdx === -1) return { aPhrase: String(combined || "").trim(), bPhrase: "" };

  const aTokens = toks.slice(0, andIdx);
  let bTokens = toks.slice(andIdx + 1);
  const aPhrase = aTokens.join(" ").trim();
  if (!bTokens.length) return { aPhrase, bPhrase: "" };
  ...
  const b0 = normalizeWord(bTokens[0]);
  const bLooksBare = !DET.has(b0) && !/(?:'s|’s)$/i.test(bTokens[0]);
  if (bLooksBare && bTokens.length <= 3) {
    const prepIdx = findLastIndex(aTokens, (t) => PREP.has(normalizeWord(t)));
    if (prepIdx !== -1) {
      const prefix = aTokens.slice(0, prepIdx + 1);
      const after = aTokens[prepIdx + 1];
      if (after) {
        const lw = normalizeWord(after);
        const possLike = DET.has(lw) || /(?:'s|’s)$/i.test(after);
        if (possLike && normalizeWord(bTokens[0]) !== lw) prefix.push(after);
      }
      bTokens = prefix.concat(bTokens);
    }
  }

  return { aPhrase, bPhrase: bTokens.join(" ").trim() };
}
...
function renderIntro() {
  ...
  const title = questions[0]?.title || "Herma L6-E3";
  const instruction =
  ...
  area.innerHTML = `
    <div class="box">
      <div style="font-size:18px; font-weight:900; color:#7e3106; margin-bottom:10px;">📘 Herma L6-E3</div>
  ...
  if (stage2Ok) {
    ...
    ab.innerHTML = `
      <div class="ab-row part-a">
        <div class="ab-card done joined-glow">${finalHtml}</div>
      </div>
    `;
...
function isTokenMatchSlotForHint(token, slot){
  if (!token || !slot || slot.kind !== "slot" || slot.filled) return false;
  if (slot.fillRule === "exact") {
    return token.lower === normalizeWord(slot.expectedLower || "");
  }
  if (slot.fillRule === "transform") {
    const expected = normalizeWord(slot.expectedLower || "");
    if (expected && token.lower !== expected) return false;
    return canApplyNounConversion(slot, token);
  }
...
function findMatchingSlotIndexForHint(token, preferredIndex){
  if (!token) return -1;
  if (Number.isInteger(preferredIndex) && preferredIndex >= 0 && preferredIndex < slots.length) {
    const s = slots[preferredIndex];
    if (isTokenMatchSlotForHint(token, s)) return preferredIndex;
  }
  for (let i = 0; i < slots.length; i++) {
    if (isTokenMatchSlotForHint(token, slots[i])) return i;
  }
  return -1;
}
function findSubsequenceLast(hay, needle){
  if (!needle.length) return -1;
...
function startNominalDragTipArc(){
  ...
  const activeTargetIndex = pickTargetSlotIndex();
  let targetIndex = activeTargetIndex;
  let preferredToken = null;

  // 우선: 하이라이트된(=verb) 토큰을 기준으로 drag 경로를 잡는다.
  const highlighted = srcTokens.filter((t) => !t.used && t.verb);
  for (const t of highlighted) {
    const matchIdx = findMatchingSlotIndexForHint(t, activeTargetIndex);
    if (matchIdx !== -1) {
      preferredToken = t;
  ...
  if (!preferredToken && targetSlot && targetSlot.kind === "slot") {
  ...
  if (!preferredToken) {
    preferredToken = srcTokens.find(t => !t.used && t.verb)
  ...
  if (preferredToken && (targetIndex < 0 || !slots[targetIndex] || slots[targetIndex].filled)) {
    const matched = findMatchingSlotIndexForHint(preferredToken, activeTargetIndex);
