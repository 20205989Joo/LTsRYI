(function initHermaL5E1Round2(global) {
  "use strict";

  if (!global || global.HermaL5E1Round2) return;

  var MODULE_NAME = "HermaL5E1Round2";
  var SCRIPT_NAME = "herma-l5e1_round2.js";
  var STYLE_ID = "herma-l5e1-round2-style";
  var INSTRUCTION_KR = "\uBB38\uC7A5\uC744 \uB4A4\uC9D1\uC5B4\uBCF4\uC138\uC694.";
  ...
  var state = {
    ...
    level: "Basic",
    day: "118",
    quizTitle: "quiz_Grammar_Basic_118_round2",
    rawRows: [],
    questions: [],
    currentIndex: 0,
    results: [],
  ...
  function renderSourceWithHighlight(sentence, targetPhrase) {
    var text = String(sentence || "");
    var phrase = String(targetPhrase || "").trim();
    ...
    if (idx < 0 && phrase.indexOf(" ") > -1) {
      var last = phrase.split(/\s+/).filter(Boolean).slice(-1)[0] || "";
      if (last) {
        lowerPhrase = last.toLowerCase();
        idx = lowerText.indexOf(lowerPhrase);
        phrase = last;
      }
    }
  ...
  function buildQuestionsFromRows() {
    ...
    state.questions = filtered.map(function (r, idx) {
      ...
      var sourceSentence = ensureEndMark(String(r["SentenceA"] || "").trim());
      ...
      return {
        ...
        title: String(r["Title"] || "").trim() || "Herma L5-E1 Round2",
        instruction: String(r["Instruction"] || "").trim() || INSTRUCTION_KR,
        sourceSentence: sourceSentence,
        flipStart: String(r["FirstWord"] || "").trim(),
      ...
      return !!q.active && !!q.sourceSentence && !!q.modelAnswer && q.answer.inputWords.length > 0;
    });
  }
  ...
  function renderIntro() {
    ...
    area.innerHTML = [
      ...
      "  </div>",
      '  <button class="r51-quiz-btn" style="width:100%; margin-top:12px;" onclick="HermaL5E1Round2.startQuiz()">Start</button>',
      "</div>"
    ].join("\n");
  }
  ...
  function renderQuestion() {
    ...
    var sourceHtml = renderSourceWithHighlight(q.sourceSentence, q.flipStart);
    ...
    area.innerHTML = [
      ...
      '    <div class="r51-stem">' + escapeHtml(LABEL_ORIGINAL_KR) + " : " + sourceHtml + "</div>",
      ...
      '    <button class="r51-quiz-btn" id="submit-btn" onclick="HermaL5E1Round2.submitAnswer()">\uC81C\uCD9C</button>',
      '    <button class="r51-quiz-btn" id="next-btn" onclick="HermaL5E1Round2.goNext()" disabled>\uB2E4\uC74C</button>',
      "  </div>",
      "</div>"
    ].join("\n");
  ...
  function submitAnswer() {
    ...
    state.results.push({
      ...
      word: "Herma L5-E1 Round2 / Q" + q.qNumber,
