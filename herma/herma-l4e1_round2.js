(function initHermaL4E1Round2(global) {
  "use strict";

  if (!global || global.HermaL4E1Round2) return;

  var MODULE_NAME = "HermaL4E1Round2";
  var SCRIPT_NAME = "herma-l4e1_round2.js";
  var STYLE_ID = "herma-l4e1-round2-style";
  var INSTRUCTION_KR = "\uD55C\uAD6D\uC5B4 \uB73B\uB9CC \uBCF4\uACE0 \uC601\uC791\uD574\uBCF4\uC138\uC694";
  ...
  var state = {
    ...
    level: "Basic",
    day: "115",
    quizTitle: "quiz_Grammar_Basic_115_round2",
    rawRows: [],
    questions: [],
    currentIndex: 0,
    results: [],
  ...
  function buildHintedAnswerParts(answerBlueprint, qNumber) {
    ...
    var hintMode = "half";
    var wordCount = Array.isArray(answerBlueprint && answerBlueprint.wordTokens)
      ? answerBlueprint.wordTokens.length
      : 0;
    var fixedWordIndexes = new Set();

    if (wordCount > 1) {
      var startIndex = Number(qNumber || 0) % 2 === 0 ? 1 : 0;
      for (var i = startIndex; i < wordCount; i += 2) {
        fixedWordIndexes.add(i);
      }

      if (!fixedWordIndexes.size) {
        fixedWordIndexes.add(0);
      }
      if (fixedWordIndexes.size >= wordCount) {
        fixedWordIndexes.delete(wordCount - 1);
      }
    }
  ...
  function buildQuestionsFromRows() {
    ...
    state.questions = filtered.map(function (r, idx) {
      ...
      var promptSentence = String(r["PromptKo"] != null ? r["PromptKo"] : "").trim() || koreanMeaning;

      return {
        no: idx + 1,
        qNumber: qNumber,
        title: String(r["Title"] || "").trim() || "Herma L4-E1 Round2",
      ...
      return !!q.active && !!(q.promptSentence || q.koreanMeaning) && q.answer.inputWords.length > 0;
    });
  }
  ...
  function renderIntro() {
    ...
    area.innerHTML = [
      ...
      "    \uD55C\uAD6D\uC5B4 \uB73B\uB9CC \uBCF4\uACE0 \uC601\uC5B4 \uBB38\uC7A5\uC744 \uC791\uC131\uD558\uC138\uC694.",
      "  </div>",
      '  <div style="font-size:12px;line-height:1.5;color:#2b2b2b;margin-bottom:8px;">',
      "    Total Questions: " + total,
      "  </div>",
      '  <button class="r32-quiz-btn" style="width:100%; margin-top:12px;" onclick="HermaL4E1Round2.startQuiz()">Start</button>',
      "</div>"
    ].join("\n");
  }
  ...
  function renderQuestion() {
    ...
    var promptKo = q.promptSentence || q.koreanMeaning || MEANING_FALLBACK_KR;
    ...
    area.innerHTML = [
      ...
      '    <div class="r32-stem">\uD55C\uAD6D\uC5B4 \uB73B : ' + escapeHtml(promptKo) + "</div>",
      "  </div>",
      renderSlotArea(q),
      '  <div class="r32-feedback" id="feedback-line"></div>',
      '  <div class="r32-btn-row">',
      '    <button class="r32-quiz-btn" id="submit-btn" onclick="HermaL4E1Round2.submitAnswer()">\uC81C\uCD9C</button>',
      '    <button class="r32-quiz-btn" id="next-btn" onclick="HermaL4E1Round2.goNext()" disabled>\uB2E4\uC74C</button>',
      "  </div>",
      "</div>"
    ].join("\n");
  ...
  function submitAnswer() {
    ...
    state.results.push({
      ...
      word: "Herma L4-E1 Round2 / Q" + q.qNumber,
