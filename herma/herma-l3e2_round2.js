(function initHermaL3E2Round2(global) {
  "use strict";

  if (!global || global.HermaL3E2Round2) return;

  var MODULE_NAME = "HermaL3E2Round2";
  var SCRIPT_NAME = "herma-l3e2_round2.js";
  var STYLE_ID = "herma-l3e2-round2-style";
  ...
  var state = {
    ...
    level: "Basic",
    day: "110",
    quizTitle: "quiz_Grammar_Basic_110_round2",
    rawRows: [],
    questions: [],
    currentIndex: 0,
    results: [],
  ...
  function buildHintedAnswerParts(answerBlueprint, qNumber) {
    ...
    var hintMode = Number(qNumber) % 2 === 1 ? "pre-comma" : "first-word";

    var lastWordIndexBeforeComma = -1;
    if (hintMode === "pre-comma") {
      var seenWords = 0;
      var foundComma = false;

      for (var i = 0; i < sourceParts.length; i++) {
        var part = sourceParts[i];
        if (part.kind === "word") seenWords += 1;
        if (part.kind === "punct" && part.value === ",") {
          foundComma = true;
          lastWordIndexBeforeComma = seenWords - 1;
          break;
        }
      }

      if (!foundComma || lastWordIndexBeforeComma < 0) {
        hintMode = "first-word";
      }
    }
    ...
    if (hintMode === "pre-comma") {
      fixedWordIndexes.add(lastWordIndexBeforeComma);
    } else if (answerBlueprint.wordTokens.length > 0) {
  ...
  function buildQuestionsFromRows() {
    ...
    state.questions = filtered.map(function (r, idx) {
      ...
      return {
        no: idx + 1,
        qNumber: qNumber,
        title: String(r["Title"] || "").trim() || "Herma L3-E2 Round2",
  ...
  function renderIntro() {
    ...
    area.innerHTML = [
      ...
      "  </div>",
      '  <button class="r32-quiz-btn" style="width:100%; margin-top:12px;" onclick="HermaL3E2Round2.startQuiz()">Start</button>',
      "</div>"
    ].join("\n");
  }
  ...
  function renderQuestion() {
    ...
    area.innerHTML = [
      ...
      '    <button class="r32-quiz-btn" id="submit-btn" onclick="HermaL3E2Round2.submitAnswer()">\uC81C\uCD9C</button>',
      '    <button class="r32-quiz-btn" id="next-btn" onclick="HermaL3E2Round2.goNext()" disabled>\uB2E4\uC74C</button>',
      "  </div>",
      "</div>"
    ].join("\n");
  ...
  function submitAnswer() {
    ...
    state.results.push({
      ...
      word: "Herma L3-E2 Round2 / Q" + q.qNumber,
