...
let level = "Basic";
let day = "107";
let quizTitle = "quiz_Grammar_Basic_107";
let userId = "";
...
function buildQuestionsFromRows() {
  ...
  questions = filtered.map((r, idx) => {
    ...
    const transformMeta = parseTransformsMetaForL2E3(transformsRaw);
    ...
    const finalHint = String(transformMeta.hint || hint || "").trim();
    const finalInsAfter = Number(transformMeta.insAfter) || insAfter || 0;

    return {
      qNumber, title, instruction,
      questionRaw, stem, A, B, hint: finalHint,
...
function parseTransformsMetaForL2E3(raw) {
  const meta = {};
...
function renderIntro() {
  ...
  const title = questions[0]?.title || "Herma L2-E3";
  const instruction = "두 문장을 약분해보세요!";

  area.innerHTML = `
    <div class="box">
      <div style="font-size:18px; font-weight:900; color:#7e3106; margin-bottom:10px;">📘 Herma L2-E3</div>
...
function renderQuestion() {
  const area = document.getElementById("quiz-area");
  if (!area) return;
  document.querySelectorAll(".b-drag-chip, .b-clause-chip, .b-handle-point").forEach((el) => el.remove());
  ...
  area.innerHTML = `
    ...
    <div class="box hidden" id="after-pass">
      <div class="answer-wrap">
        <textarea id="user-english" class="eng" rows="3" placeholder="한 문장으로 합쳐보세요! ex) This is the book which I bought yesterday"></textarea>
  ...
  function enterFrontDragStage() {
    ...
    if (!sb) return;
    startFrontDragStep(sb, () => {
      frontMoved = true;
      frontDragActive = false;
      bFadedIdxSet = new Set();
      sb.innerHTML = buildBFrontReductionHTML(tokensB, bFadedIdxSet);
      toastOk("위치 이동 완료!");
      enterReduceStage();
    });
  }
  ...
  function enterHandleReadyStage() {
    const pill = document.getElementById("stage-pill");
    const it = document.getElementById("instruction-text");
    const sub = document.getElementById("instruction-sub");
  ...
  function startFrontDragStep(sb, onDone) {
    const reqSpans = Array.from(sb.querySelectorAll('[data-req="1"]'));
    const reqText = getReqPhraseText(tokensB);
    if (!reqSpans.length || !reqText) {
      onDone?.();
      return;
    }

    const groupRectBefore = getSpanGroupRect(reqSpans);

    const slot = document.createElement("span");
    slot.className = "b-front-slot";
    slot.setAttribute("aria-hidden", "true");
    slot.style.width = `${Math.max(78, Math.round(groupRectBefore.width))}px`;
    sb.prepend(slot);

    // 슬롯 삽입으로 req 토큰들이 오른쪽으로 밀린 실제 좌표를 다시 반영한다.
    const groupRectAfter = getSpanGroupRect(reqSpans);
    const shiftX = groupRectAfter.left - groupRectBefore.left;
    const shiftY = groupRectAfter.top - groupRectBefore.top;

    const chip = document.createElement("div");
    chip.className = "b-clause-chip";
    chip.textContent = reqText;
    document.body.appendChild(chip);
    const chipRect0 = chip.getBoundingClientRect();
    const startX = Math.round(groupRectBefore.left + shiftX + ((groupRectBefore.width - chipRect0.width) / 2));
    const startY = Math.round(groupRectBefore.top + shiftY + ((groupRectBefore.height - chipRect0.height) / 2));
    ...
    if (tapTip) {
      ...
      startTapTipArc(
        tapTip,
        { x: groupRectAfter.left + groupRectAfter.width / 2, y: groupRectAfter.top + (groupRectAfter.height / 2) },
  ...
  async function spawnHandlePointAndEnableDrag(sa, sb, bArea, q, engEl) {
    ...
    if (reqSpans.length) {
      const rr = getSpanGroupRect(reqSpans);
  ...
  function enterTranslateMode(q) {
    ...
    if (plain) {
      const configuredFinalParts = parseLaststageFinalSentenceForL2E3(q.laststageFinalSentence);
      if (configuredFinalParts.length) {
        plain.innerHTML = `<div>${renderConfiguredFinalSentenceForL2E3(configuredFinalParts)}</div>`;
      } else {
        const after = correctInsAfter || 2;
        const rest = stripTrailingPeriod(getReducedClauseText(tokensB));
        const hint = getHintWord(q.hint || "which / where");
        plain.innerHTML = `
...
function buildFinalLineHTML(tokensA, afterPos, hintWord, bRestText){
  const hint = escapeHtml(getHintWord(hintWord || "which"));
  const rest = stripTrailingPeriod(String(bRestText || "").trim());
...
function parseLaststageFinalSentenceForL2E3(raw) {
  const s = String(raw || "").trim();
  if (!s) return [];
...
function mapFinalSegClassForL2E3(seg) {
...
function renderConfiguredFinalSentenceForL2E3(parts) {
  const chunks = parts.map((part) => {
    const text = String(part?.text || "").trim();
    if (!text) return "";
    const cls = mapFinalSegClassForL2E3(part.seg);
...
function parseLaststageKRTokensForL2E3(raw) {
  const s = String(raw || "").trim();
  if (!s) return [];
...
function mapKRTokensSegToRoleForL2E3(seg) {
  const s = String(seg || "").toLowerCase();
...
function tokenRoleClassForL2E3(role) {
  if (role === "A") return "uA";
  if (role === "B") return "uB";
...
function normalizeSentenceSpacing(s) {
  return String(s || "")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/\s+([.,!?])/g, "$1");
...
function getReqPhraseText(tokensB) {
  return normalizeSentenceSpacing(concatTokens(tokensB, (t) => t.isReq));
}

function buildBFrontReductionHTML(tokensB, fadedSet = new Set()) {
  let reqHtml = "";
  let restHtml = "";
  ...
  const req = normalizeSentenceSpacing(reqHtml);
  const rest = normalizeSentenceSpacing(restHtml);
...
function revealTranslate(q) {
  isKoLocked = false;
  const configured = parseLaststageKRTokensForL2E3(String(q?.laststageKRTokens || "").trim());
  if (configured.length) {
    bankTokens = shuffleArray(configured.map((t, i) => ({
      id: `t${i}_${t.text}`,
      text: t.text,
      role: mapKRTokensSegToRoleForL2E3(t.seg),
    })));
  } else {
...
function renderBankAndAnswer() {
  ...
  if (window.HermaKRScramble?.render) {
    window.HermaKRScramble.render({
      ...
      decorateToken: (el, tok) => {
        if (!el || !tok) return;
        const cls = tokenRoleClassForL2E3(tok.role);
  ...
  bankTokens.forEach((tok) => {
    ...
    const roleClass = tokenRoleClassForL2E3(tok.role);
