...
const PAGE_LABEL = "Aisth L5-E1";
const MAX_QUESTIONS = 0; // 0 = unlimited

const DEFAULT_INSTRUCTION = "올바른 불규칙 동사형을 골라보세요.";
...
let level = "aisth";
let day = "016";
let quizTitle = "quiz_Grammar_aisth_l5e1";
let userId = "";
...
function parseChoiceQuestion(raw) {
  ...
  for (const line of lines) {
    ...
    if (m) {
      options.push({ label: m[1].toUpperCase(), text: String(m[2] || "").trim() });
      continue;
    }
  ...
  if (!options.length) {
    ...
    while ((m = re.exec(text)) !== null) {
      options.push({ label: String(m[1] || "").toUpperCase(), text: String(m[2] || "").trim() });
    }
  }
