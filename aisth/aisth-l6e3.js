...
let level = "aisth";
let day = "022";
let quizTitle = "quiz_Grammar_aisth_l6e3";
let userId = "";
...
function renderQuestion() {
  ...
  const itemsHtml = group.map((q, idx) => {
    ...
    const inputHtml = q.type === "blank"
      ? `<input id="user-answer-${idx}" class="short-input" type="text" autocomplete="off" placeholder="${escapeHtmlAttr(placeholder)}" />`
      : `<textarea id="user-answer-${idx}" rows="2" placeholder="${escapeHtmlAttr(placeholder)}"></textarea>`;
    const hintHtml = q.verbHint ? `<div class="hint-base">동사원형 힌트: <code>${escapeHtml(q.verbHint)}</code></div>` : "";
  ...
  if (inputs.length) {
    ...
    inputs.forEach((input, idx) => {
      ...
      input.addEventListener("keydown", (ev) => {
        if (q.type === "blank" && ev.key === "Enter") {
          ev.preventDefault();
          submitCurrentAnswer();
          return;
        }
        if (q.type !== "blank" && ev.key === "Enter" && ev.ctrlKey) {
          ev.preventDefault();
          submitCurrentAnswer();
        }
      });
...
function submitCurrentAnswer() {
  ...
  const attempts = group.map((q, idx) => {
    const input = document.getElementById(`user-answer-${idx}`);
    return {
      q,
      input,
  ...
  if (attempts.some((a) => !a.value)) {
    showToast("no", TEXT.INPUT_REQUIRED);
    return;
  }
  const allOk = attempts.every((a) => isAnswerCorrect(a.q.type, a.value, a.q.answer));
